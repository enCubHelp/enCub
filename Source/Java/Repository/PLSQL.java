...
public class PLSQL
{
    ...
    public void Select(String parmType, String parmProject, String parmPLSQL)
    {
        ...
        try
        {
            ...
            {
                _sql = "SELECT DISTINCT Project, PLSQL FROM PLSQL ORDER BY Project ASC, PLSQL ASC";
            }
            ...
            while(_rs.next())
            {
                if(parmType.equals("ALL"))
                {
                    _selectList.add(_rs.getString("Project") + ":" + _rs.getString("PLSQL"));
                }
                else if(parmType.equals("DISTINCT"))
                {
                    _selectList.add(_rs.getString("PLSQL"));
                }
                else
                {
    ...
    public void Delete(String parmProject, String parmPLSQL, int parmSeq)
    {
        ...
        try
        {
            ...
            {
                _statement.executeUpdate(_sql);
                _sql = "DELETE ConversionLog WHERE Project = '" + parmProject + "' AND PLSQL = '"  + parmPLSQL   + "';";
                _statement.executeUpdate(_sql);
                _sql = "DELETE Java WHERE Project = '" + parmProject + "' AND PLSQL = '"  + parmPLSQL   + "';";
                _statement.executeUpdate(_sql);
                _sql = "DELETE Parsingerrlog WHERE Project = '" + parmProject + "' AND PLSQL = '"  + parmPLSQL   + "';";
                _statement.executeUpdate(_sql);
                _sql = "DELETE ParsingTbl WHERE Project = '" + parmProject + "' AND PLSQL = '"  + parmPLSQL   + "';";
                _statement.executeUpdate(_sql);
                _sql = "DELETE PlsqlCrud WHERE Project = '" + parmProject + "' AND PLSQL = '"  + parmPLSQL   + "';";
                _statement.executeUpdate(_sql);
                _sql = "DELETE Tokens WHERE Project = '" + parmProject + "' AND PLSQL = '"  + parmPLSQL   + "';";
                _statement.executeUpdate(_sql);
                _sql = "DELETE Java WHERE Project = '" + parmProject + "' AND PLSQL = '"  + parmPLSQL   + "';";
                _statement.executeUpdate(_sql);
            }
    ...
    public static void main(String[] args) throws Exception
    {
           ...
           args[1].equals("ALL"))
        {
            ...
            {
                String _getString[] = _selectList.get(_loop).split(":");
                System.out.println("Project=" + _getString[0]);
                System.out.println("PLSQL="   + _getString[1]);
            }
        }
                ...
                args[1].equals("DISTINCT"))
        {
            ...
            for(int _loop = 0; _loop < _selectList.size(); _loop++)
            {
                System.out.println("Project=" + args[2]);
                System.out.println("PLSQL="   + _selectList.get(_loop));
            }
       }
        else if(args.length >= 3 &&
                args[0].equals("SELECT"))
        {
            ...
            for(int _loop = 0; _loop < _selectList.size(); _loop++)
            {
                System.out.println("Project=" + args[1]);
                System.out.println("PLSQL="   + args[2]);
                System.out.println("Seq="     + (_loop + 1));
                System.out.println("Text="    + _selectList.get(_loop));
            }
        }
