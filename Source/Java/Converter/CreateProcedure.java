...
public class CreateProcedure extends PLSQLStatement {
            ...
            while(true)
            {
                if (GetCurrentRule() == null)
                {
                    break;
                         ...
                         GetCurrentRule().equals("create_procedure"))
                {
                    break;
                }
                ...
                else if (GetCurrentRule().equals("create_procedure") &&
                         GetCurrentTokenName().equals(""))
                {
                    GetNextToken();                                         // CREATE
                    while(!GetCurrentTokenName().equals("PROCEDURE"))       // skip OR REPLACE
                    {
                        GetNextPosition();
                    }
                    GetNextPosition();                                      // skip PROCEDURE
                    ...
                    ConvertStatement(JavaWriter._Procedure, GetCurrentLine(), GetCurrentColumn(), JavaWriter._CMD_Class, "public static void " + _procedureName + "(");
                    GetNextPosition();
                    GetNextTokenName();
                    if (GetCurrentTokenName().equals(_leftParen))
                    {
                        ...
                        while(!GetCurrentTokenName().equals(_rightParen))
                        {
                            ...
                            else if (GetCurrentTokenName().equals(_comma))
                            {
                                ...
                                ConvertStatement(JavaWriter._Procedure, _sourceLines, _sourceColumns, JavaWriter._CMD_Parameter, _statement);
                                _parameter      = "";
                                _datatype       = "";
                                _length         = 0;
                                _precision      = 0;
                        ...
                        _statement      = "";
                         ...
                         GetCurrentTokenName().equals("AUTHID"))
                {
                    _tokenName = GetNextTokenName();
                    GetNextPosition();
                    if ((_tokenName = GetNextTokenName()).equals("CURRENT_USER"))
                    {
                        System.out.println("AUTHID CURRENT_USER is not supported.");
                    }
                    else if ((_tokenName = GetNextTokenName()).equals("DEFINER"))
                    {
                        System.out.println("AUTHID DEFINER is not supported.");
                    }
                }
                else if (GetCurrentRule().equals("create_procedure") &&
...
//////                            System.out.println("<<<<<<<<<<<<<<<<<<<<< VARIABLE_NAME >>>>>>>>>>>>>>>>>>>>>>");
                            ...
                            System.out.println("CreateProcedure:Variable=[" + _parameter + "]");
                            GetNextPosition();
                            GetNextRule();
                            System.out.println("CreateProcedure:GetCurrentRule()=[" + GetCurrentRule() + "]");
                            if (GetCurrentRule() != null &&
                            ...
                            System.out.println("CreateProcedure:Variable=[" + _parameter + "],_datatype=[" + _datatype + "]");
                            GetNextPosition();
                            GetNextTokenName();
                            System.out.println("CreateProcedure:GetCurrentTokenName()=[" + GetCurrentTokenName() + "]");
                            ...
                            System.out.println("CreateProcedure:Variable=[" + _parameter + "],_datatype=[" + _datatype + "],_length=[" + _length + "],_precision=[" + _precision + "],_initValue=[" + _initValue + "]");
                            while(true)
                            {
                                GetNextPosition();
                                if (GetCurrentTokenName() == null ||
