...
public class PLSQL_TConverter {
    public void Convert(String parmFile) throws Exception {
        ...
        List<Token> _tokenList = new ArrayList<Token>();
        int _index = 0;
             ...
             _token = _lexer.nextToken())
        {
            ...
            System.out.printf("LINE %d Column %d - TOKEN %d %s\n", _token.getLine(), _token.getCharPositionInLine(), _token.getType(), _token.getText());
            _index++;
        }
        System.out.println("Size = [" + _tokenList.size() + "]");
        for(int _loop = (_tokenList.size() - 1); _loop >= 2; _loop--)
        {
//////            if (tryParseInt(_tokenList.get(_loop).getText()))
//////            {
//////                System.out.println("Integer = [" + _tokenList.get(_loop).getText() + "]");
//////                if (_tokenList.get(_loop - 1).getText().equals("."))
//////                {
//////                    System.out.println("Next = \".\"");
//////                }
//////                else
//////                {
//////                    System.out.println("_tokenList.get(_loop - 1).getText()=[" + _tokenList.get(_loop - 1).getText() + "]");
//////                }
//////            }
            if (tryParseInt(_tokenList.get(_loop).getText()) &&
                _tokenList.get(_loop - 1).getText().equals(".") &&
                tryParseInt(_tokenList.get(_loop - 2).getText()))
            {
                System.out.println("Number = [" + _tokenList.get(_loop - 2).getText() + _tokenList.get(_loop - 1).getText() + _tokenList.get(_loop).getText() + "]");
            }
        }
        ...
        //////{
        //////    _tokens.consume();
        //////} while (_tokens.LA(1) != -1);
        //////List<Token> _tokenList = _tokens.getTokens();
        //////_tokenList = _tokens.getTokens();
        //////List<String> _block = new ArrayList<String>();
        //////for(int i = 0; i < _tokenList.size(); i++)
        //////{
        //////    if(_token.getText().equals("IF"))
        //////    {
        //////        _block.add(_token.getText());
        //////    }
        //////    else if(_token.getText().equals("END") &&
        //////            _block.size() > 0)
        //////    {
        //////        _block.remove(_block.size() - 1);
        //////    }
        //////}
        //////int _ifCnt = 0;
        //////for (Token _token : _tokenList)
        //////{
        //////    if(_token.getText().equals("IF"))
        //////    {
        //////        _ifCnt ++;
        //////    }
        //////    int _type = _token.getType();
        //////    int _line = _token.getLine();
        //////    int _column = _token.getCharPositionInLine();
        //////    String _text = _token.getText();
        //////    System.out.printf("LINE %d, Column %d - TOKEN %d %s\n", _line, _column, _type, _text);
        //////}
        ...
        _input = new ANTLRInputStream(_is);
        _lexer = new PLSQL_TLexer(_input);
        CommonTokenStream _tokens = new CommonTokenStream(_lexer);
        PLSQL_TParser _parser = new PLSQL_TParser(_tokens);

        /*
        ParserTreeWalker _worker = new ParserTreeWorker();
        PLSQLListener _listener = new PLSQLListener();
        walker.walk(_listener, _parser.start());
    ...
    public static void main(String[] args) throws Exception {
        if ( args.length > 0 )
        {
            PLSQL_TConverter _converter_t = new PLSQL_TConverter();
            _converter_t.Convert(args[0]);
        }
    }
    //////private class PLSQLListener extends PLSQL_TBaseListener {
    //////    public void enterInsert_into_clause(PLSQL_TParser.Insert_into_clauseContext ctx) {
    //////        super.enterInsert_into_clause(ctx);    //To change body of overridden methods use File | Settings | File Templates.
    //////        System.out.println("ENTER Insert_into_clause: " + ctx.getText());
    //////    }
    //////}
}
